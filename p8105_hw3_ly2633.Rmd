...
merged_data =
  left_join(accel_df, Covar_df, by = "seqn") ### ??? why do I not see demographics variables
...
# incorrect
mutate(
    sex = factor(sex, levels = c("Male", "Female")),  # Encode gender as a factor
    race = factor(race),  
    education = factor(education, ordered = TRUE),  # Convert education to an ordered factor, if applicable
    income = factor(income, ordered = TRUE),  # Convert income to an ordered factor if it's categorical
