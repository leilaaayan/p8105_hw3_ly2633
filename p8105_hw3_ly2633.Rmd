...
total_activity_data <- cleaned_data %>%
  ...
  summarise(total_activity = sum(activity_variable, na.rm = TRUE), .groups = 'drop')  # Replace 'activity_variable' with the actual activity column name
...
```
Answers:
The plot shows how median ride duration varies by day of the week, month (comparing January with other months), and year (2020 versus 2024). In 2020, there is a noticeable increase in ride duration on the weekends, with Saturday and Sunday showing the highest median ride times, peaking at around 16 minutes. In contrast, weekdays exhibit lower and more consistent durations, particularly on Fridays and Mondays. By 2024, while the weekend trend persists, the overall ride durations have decreased slightly, especially on Sundays, and the difference in duration between weekdays and weekends is less pronounced. January consistently shows lower median ride durations across both years, with minimal variation across the days of the week, indicating that rides are generally shorter during colder months. This suggests a seasonal pattern, where longer rides are more common in warmer months, particularly on weekends. Additionally, the slight decrease in median ride duration between 2020 and 2024 could reflect changes in riding behavior, such as route preferences, weather, or other external factors.
